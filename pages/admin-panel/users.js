...
const fetcher = () =>
  fetch("https://rentify-api.runflare.run/api/auth/admin/users", {
    method: "GET",
...
const Users = () => {
  ...
  const deleteUser = (id) => {
    fetch(`https://rentify-api.runflare.run/api/auth/admin/users/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${getCookie("token")}` },
    })
      .then((res) => res.json())
  ...
  const changeUserRoleHandler = (id, role) => {
    ...
    fetch(`https://rentify-api.runflare.run/api/auth/admin/change-role/${id}`, {
      method: "PUT",
      headers: {
